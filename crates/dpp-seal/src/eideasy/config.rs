...
pub const PROVIDER: &str = "eideasy";
...
pub const ENV_GROUP: &str = "SEAL_EIDEASY_*";
...
pub fn any_env_set(get: &impl Fn(&str) -> Option<String>) -> bool {
    [ENV_BASE_URL, ENV_CLIENT_ID, ENV_HMAC_KEY]
        .iter()
...
impl EideasyConfig {
    ...
    pub fn resolve(get: impl Fn(&str) -> Option<String>) -> Result<Self, SealError> {
        ...
        let hmac_key = read(ENV_HMAC_KEY);
        let missing: Vec<&str> = [
        ...
        Ok(Self {
            ...
            signature_profile: read(ENV_SIGNATURE_PROFILE).unwrap_or_else(|| {
