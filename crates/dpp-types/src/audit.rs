//! Audit trail wire type, hash-chain verification, and persistence port.
//!
...
///
/// `#[serde(deny_unknown_fields)]` — an unknown field here must fail loudly,
...
impl AuditEntry {
    ...
    /// Takes a plain actor string — `AuthContext`-aware call sites pass
    /// `&auth.user_id` (single-tenant: no operator scope to include,
    /// DECISION-0002).
    pub fn new(
        passport_id: &str,
        action: &str,
