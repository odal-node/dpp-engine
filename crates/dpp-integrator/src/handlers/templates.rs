...
fn templates() -> Vec<Template> {
    let mut out = vec![
        ...
        Template {
            key: "mattress",
            source: Source::Embedded(include_str!("../../templates/mattress-v1.csv")),
            filename: "odal-mattress-template.csv",
        },
        Template {
            key: "furniture",
            source: Source::Embedded(include_str!("../../templates/furniture-v1.csv")),
            filename: "odal-furniture-template.csv",
        },
        Template {
            key: "toy",
            source: Source::Embedded(include_str!("../../templates/toy-v1.csv")),
            filename: "odal-toy-template.csv",
        },
        Template {
            key: "construction",
            source: Source::Embedded(include_str!("../../templates/construction-v1.csv")),
            filename: "odal-construction-template.csv",
        },
    ];
...
/// The CSV template served under `key`, or `None` if nothing is served there.
///
/// Separate from the handler so the drift tests in `domain::validate` can read
/// what an operator would actually download. A template nobody can read from a
/// test is a template nothing can check.
///
/// Takes a **template key**, not a product group — since batteries these are no
/// longer the same thing. `battery-ev` is a key; `battery` is a product group
/// and is served by nothing. Reads [`templates()`] rather than carrying its own
/// lookup, because that list previously had three homes that disagreed and a
/// second lookup beside it would re-create the problem.
///
/// Returns `Cow` because half the templates are committed files borrowed at
/// compile time and half are rendered per call.
#[must_use]
pub fn template_for(key: &str) -> Option<Cow<'static, str>> {
    templates()
        .iter()
        .find(|t| t.key == key)
        .map(|t| t.source.render())
}

/// Every key this endpoint serves, for tests that need to walk the real set.
#[must_use]
