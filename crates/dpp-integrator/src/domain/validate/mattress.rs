//! Mattress row validation.

use std::collections::HashMap;

use dpp_domain::{
    passport::ManufacturerInfo,
    product_group::{MattressData, ProductGroup, ProductGroupData},
};
...
/// Validate a single mattress row and convert it to a vault
/// `CreatePassportRequest`.
///
/// The schema's required set is small — `gtin`, `primaryMaterial`,
/// `countryOfOrigin` — because the mattress act is at an earlier stage than the
/// battery or textile ones. The optional columns are carried anyway: a row that
/// states them produces a passport that states them, and an importer that
/// silently dropped columns an operator filled in would be worse than one that
/// never offered them.
pub fn validate_mattress_row(
    ...
    Ok(CreatePassportRequest {
        ...
        product_group: Some(ProductGroup::Mattress),
