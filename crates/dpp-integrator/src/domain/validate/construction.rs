//! Construction-product row validation.

use std::collections::HashMap;

use dpp_domain::{
    passport::ManufacturerInfo,
    product_group::{ConstructionData, ProductGroup, ProductGroupData},
};

use crate::domain::fields::{
    optional_bool, optional_commodity_code, optional_date, optional_f64, optional_str, parse_gtin,
    require_f64, require_str,
};
...
pub(super) const COLUMNS: &[Column] = &[
    ...
    Column::required("productFamily"),
    Column::required("countryOfOrigin"),
    Column::required("co2ePerFunctionalUnitKg"),
    Column::required("functionalUnit"),
    Column::optional("recycledContentPct"),
    Column::optional("epdUrl"),
    Column::optional("ceMarking"),
];

/// Validate a single construction-product row and convert it to a vault
/// `CreatePassportRequest`.
///
/// `functionalUnit` is required alongside `co2ePerFunctionalUnitKg` because the
/// figure is meaningless without it: 620 kg CO2e per tonne of cement and per
/// cubic metre of concrete are different claims, and a number whose unit is
/// implied is a number a reader has to guess at.
pub fn validate_construction_row(
    ...
    let product_family = require_str(row, "productFamily", row_num, &mut errors);
    let country_of_origin = require_str(row, "countryOfOrigin", row_num, &mut errors);
    let co2e = require_f64(row, "co2ePerFunctionalUnitKg", row_num, &mut errors);
    let functional_unit = require_str(row, "functionalUnit", row_num, &mut errors);
    let recycled = optional_f64(row, "recycledContentPct", row_num, &mut errors);
    let epd_url = optional_str(row, "epdUrl");
    let ce_marking = optional_bool(row, "ceMarking", row_num, &mut errors);
    ...
    Ok(CreatePassportRequest {
        ...
        product_group: Some(ProductGroup::Construction),
        ...
        product_group_data: Some(ProductGroupData::Construction(ConstructionData {
            ...
            country_of_origin: country_of_origin
                .expect("field verified present by errors.is_empty() guard above"),
            co2e_per_functional_unit_kg: co2e
                .expect("field verified present by errors.is_empty() guard above"),
